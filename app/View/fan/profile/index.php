        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <div class="flex items-center justify-center md:justify-start">
            ...
            <span class="ml-2 text-gray-600">
              <span class="font-medium text-gray-800">Member since:</span> <?php echo date("Y", strtotime(user()->getCreatedAt()))?>
            </span>
          </div>
          <div class="flex items-center justify-center md:justify-start">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <span class="ml-2 text-gray-600">
              <span class="font-medium text-gray-800">Favorite Team:</span> <?php echo user()->getEmail() ?>
            </span>
          </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div class="group bg-gradient-to-br from-emerald-50 to-green-50 rounded-xl p-6 shadow-md transition-all duration-300 hover:shadow-lg hover:scale-105">
        <div class="flex items-center justify-between mb-4">
          ...
          <span class="text-2xl font-bold text-emerald-600"><?php echo $favorite_count ; ?>+</span>
        </div>
        <h3 class="text-lg font-semibold text-gray-800">Enthusiastic Fan</h3>
        <p class="text-gray-600 text-sm mt-2">Liked <?php echo $favorite_count ; ?>+ cycling moments</p>
      </div>
      <?php }?>
      <!-- Comments Badge -->
      <?php if($favorite_count>9){ ?>
      ...
      <div class="group bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 shadow-md transition-all duration-300 hover:shadow-lg hover:scale-105">
        <div class="flex items-center justify-between mb-4">
          ...
          <span class="text-2xl font-bold text-emerald-600"><?php echo $comment_count ; ?>+</span>
        </div>
        <h3 class="text-lg font-semibold text-gray-800">Active Supporter</h3>
        <p class="text-gray-600 text-sm mt-2">Posted <?php echo $comment_count ; ?>+ engaging comments</p>
      </div>
      <?php }?>
      ...
      <?php
        ...
        $today = new DateTime(date('Y-m-d')) ;
         $interval = date_diff($createdAt, $today);
     ...
     if ($years>1) {
        ...
        <div class="flex items-center justify-between mb-4">
          ...
          <span class="text-2xl font-bold text-emerald-600"><?=date("Y", strtotime(user()->getCreatedAt())); ?></span>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div class="group bg-gradient-to-br from-emerald-50 to-green-50 rounded-xl p-6 shadow-md transition-all duration-300 hover:shadow-lg hover:scale-105">
        <div class="flex items-center space-x-4">
          <img src="<?php echo $value->getphoto() ; ?>" alt="Cyclist Name" class="w-20 h-20 rounded-full object-cover border-2 border-emerald-500">
          <div>
            <h3 class="text-lg font-semibold text-gray-800"><?php echo $value->getfirst_name() . $value->getlast_name() ; ?></h3>
            <p class="text-emerald-600 font-medium"><?php echo $value->getname() ; ?></p>
          </div>
        </div>
        <div class="mt-4">
